...
public class Minesweeper_Feld {
		...
		for (int i = x - 1; i <= x + 1; i++) {
			for (int j = y - 1; j <= y + 1; j++) {
				if (i >= 0 && i < width && j >= 0 && j < height && !(i == x && j == y)) {
					if (field[i][j] == Feld.bombe || field[i][j] == Feld.flagge_bombe) {
						nrBombs++;
					}
				}
			}
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (field[x][y] == Feld.offen_keine_bombe) {
					printField(x, y);
				} else if (field[x][y] == Feld.flagge_bombe || field[x][y] == Feld.flagge_keine_bombe) {
					System.out.print("f");
				} else {
					System.out.print(".");
				}
	...
	public static void main(String[] args) {
		...
		do {
			...
			System.out.println("Bitte geben Sie Ihre Koordinaten ein. zB.: \"2 2\"");
